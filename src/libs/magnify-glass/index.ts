...
export class MagnifyGlass {
    ...
    _createElement() {
        ...
        cropBox.style.width = this.initialSize.width / this.zoomRatio + 'px'
        cropBox.style.height = this.initialSize.height / this.zoomRatio + 'px'
        ...
        dragBox.style.width = "calc(100% - " + this.zoomSpacing * 2 + 'px)'
        dragBox.style.height = "calc(100% - " + this.zoomSpacing * 2 + 'px)'
    ...
    mount = () => {
        ...
        this.magnifier.addEventListener('mousedown', this._onZoomStart)
    }
    destroy = () => {
        // 恢复视窗
        ...
        this.magnifier.removeEventListener('mousedown', this._onZoomStart)
    ...
    _onZoomStart = (event) => {
        ...
        this.isZoomTopLeft = this.originalOffset.x <= this.zoomSpacing ||
            this.originalOffset.y <= this.zoomSpacing

        this.container?.addEventListener('mousemove', this._onZooming)
        this.container?.addEventListener('mouseup', this._onZoomEnd)
        this.container?.addEventListener('mouseleave', this._onZoomEnd)
    }
    _onZooming = (event) => {
        if (!this.magnifier || !this.cropBox || !this.screenshots || !this.scaleImg)
            return
        ...
        // 更新裁剪区域的尺寸和位置
        const cropBoxW = targetWidth / this.zoomRatio
        const cropBoxH = targetHeight / this.zoomRatio
    ...
    _onZoomEnd = () => {
        this.isZooming = false
        this.container.removeEventListener('mousemove', this._onZooming)
        this.container.removeEventListener('mouseup', this._onZoomEnd)
        this.container.removeEventListener('mouseleave', this._onZoomEnd)
        this._updateScaleImg()
    }
    _updateScaleImg = () => {
        // 异常处理
        ...
        const croppedW = cropBoxW * this.zoomRatio
        const croppedH = cropBoxH * this.zoomRatio
        const croppedOffsetX = cropBoxOffsetX * this.zoomRatio
        const croppedOffsetY = cropBoxOffsetY * this.zoomRatio
